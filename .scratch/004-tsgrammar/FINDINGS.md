# Phase 2 — tsgrammar core: Findings & Verdict
...
`src/tsgrammar/` (IR, builder, analyzer, pipeline, language, conflicts) with
pytest tests in `tests/`. Re-run:

```bash
devenv shell -- python .scratch/004-tsgrammar/experiment_a.py
devenv shell -- python .scratch/004-tsgrammar/experiment_b.py
...
Editable install: `uv pip install -e .` works; `src/tsgrammar` added to the
hatch wheel packages in `pyproject.toml`. `import tsgrammar` works in the
...
`evidence/` holds the raw CLI output. The experiment (`.scratch/004-tsgrammar/

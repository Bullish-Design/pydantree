...
> This is the **bet-#1 experiment**, building on the real `src/tsgrammar/`
> package from Phase 2. Reference docs live in `.scratch/`; findings go in
> `.scratch/005-tsgrammar-glr/FINDINGS.md`.
...
Phase 1 proved Product A's model-only surface; Phase 2 (`.scratch/004-tsgrammar/`,
`src/tsgrammar/`) proved Product B's core mechanics at full schema fidelity —
...
- **Phase 2 (done, THIS is your foundation):** `.scratch/004-tsgrammar/` — the
...
2. **`.scratch/004-tsgrammar/FINDINGS.md`** — Phase 2's verdict and durable
   facts. Especially:
...
3. **Phase-2 code (skim — this is your base, extend it):**
   - `src/tsgrammar/builder.py` — the DSL surface you will extend. Today it has
   ...
   - `src/tsgrammar/conflicts.py` — the remapping (per-rule sites; Phase 3 adds
     per-production sites and the fix loop).
   - `src/tsgrammar/checks.py`, `pipeline.py`, `grammar.py` — analyzer, cache,
     IR.
   - `.scratch/004-tsgrammar/filtlang.py` — the Phase-2 hand-rolled precedence
   ...
   - `.scratch/004-tsgrammar/experiment_b.py` — the end-to-end pattern and the
...
- **wasm, packaging/wheels, package splitting** (`tscore`/`tsquery`/`tsgrammar`),
...
2. **Verified facts (don't re-derive):** tree-sitter CLI 0.25.3, bindings 0.26.0
   ...
   pruned. All in `src/tsgrammar/pipeline.py` + Phase-2 FINDINGS appendix.
3. **Where the code goes:** extend **`src/tsgrammar/`** (builder.py, checks.py,
   ...
   (`uv pip install -e .`) — `import tsgrammar` works. **Do not touch
   `src/pydantree/`.** The new work lives under
   `.scratch/005-tsgrammar-glr/` (experiments, the Run-1 grammar, evidence/).
...
- **Commit after each meaningful step** (e.g.
  `tsgrammar: precedence ladder helper emits int ladders, renumbers on insert`,
  `tsgrammar: ExpressionGrammar from a table (layering + postfix handled)`,
  `tsgrammar: ambiguity opt-in synthesizes conflicts + PREC_DYNAMIC`,
  `tsgrammar: per-production conflict sites + fix-one-rerun loop`,
  `tsgrammar: Phase-3 experiment — bet #1 feel verdict, evidence captured`).
- **Write findings as you go** into `.scratch/005-tsgrammar-glr/FINDINGS.md`.
...
- **Don't fake the primary experiment.** Run 1's corpus, Run 2's conflicts, and
  ...
  raw generator output verbatim under `.scratch/005-tsgrammar-glr/evidence/`.
- **Ask before expanding scope** beyond this brief.

## Deliverables (end of session)

1. Working Phase-3 extensions in `src/tsgrammar/` (ladder helper,
...
2. Demonstrated, with evidence:
   ...
   - (c) Run 3: the same grammar hand-rolled the Phase-2 way, with the
     comparison table.
3. `.scratch/005-tsgrammar-glr/FINDINGS.md` answering at minimum:

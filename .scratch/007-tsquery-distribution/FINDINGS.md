...
over the wheel. Two honest caveats: tscore was NOT actually B-free at import
...
# A: consume it B-free (no tsgrammar in the process — verified)
...
parallel it (`semantic_smoke` now delegates to `tsgrammar.corpus`; no parallel
machinery).
...
`tscore.loader.load_bundle`, the shared loading contract, CONCEPT §8). A
separate subprocess with tsgrammar genuinely removed from `sys.path` (a
`sitecustomize` strips the editable `src/` install; the consumer asserts
`import tsgrammar` fails) consumed the bundle via `Language.load_bundle` in
...
- **the leak the boundary exposed:** `tscore.schema` imported
  `tsgrammar.grammar` at MODULE level (the comment claimed "lazy import" —
  it wasn't), so `import tscore` was NOT B-free. Fixed: the exact-path
  derivation moved to `tscore._ir_derive`, imported only when
  `derive_from_ir` is actually called (B-side only). `import tscore` /
  `import tsquery` now never touch tsgrammar — verified by the B-free
  subprocess.
...
`tscore`, the shared contract, so B and A and the bundle's `loader.py` all use
...
`tscore` + `tree_sitter`). **The seam does not leak.**
...
| item | status | note |
|---|---|---|
| corpus harness (`tsgrammar/corpus.py`) | **LANDED** | `Corpus`/`corpus_case` + runner + renderers + snapshots; `semantic_smoke` delegates; qfilter corpus; 9 tests |
| artifact packaging + loader | **LANDED** | `BuildResult.package()`; `tscore.loader` (the shared contract); `Language.load_bundle()` one-liner; **the B-free-import fix** (`tscore._ir_derive` split) |
| community-schema tool | **LANDED** | `tsgrammar.schema_tool`; agrees with `derive_from_ir`; B-free community consumer over the wheel |
...
| external-scanner escape hatch | **LANDED** | `ExternalScannerRequiredError` (clear, names the externals, before gcc link failure); scanner.c content-addressed in the cache key; `g.external(*tokens)`; the canonical INDENT/DEDENT/NEWLINE scanner ships (`tsgrammar.scanners.indent_scanner_path()` — the pymini seed) with the pymini grammar. The scanner library beyond the one seed stays Phase-6. |
...
6. **The B-free boundary is the honest test of the A/B seam**: in-process
   ...
   asserting `import tsgrammar` fails) reveals import-time coupling.
...
9. **The bundle is one artifact**: grammar.so (export symbol recorded in the
   ...
   tree-sitter.json + a 7-line loader delegating to tscore's shared loading

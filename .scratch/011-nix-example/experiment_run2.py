...
shapes, byte-identical.
  ...
  2. a FRESH venv with ONLY the light wheels + tree-sitter-nix from the REAL
     ...
     grammar version consistency was resolved in Run 1);
  3. the community bundle built in-repo (B available);
  4. the SAME consumer (consumer_nix.py) runs in three shapes:
...
Evidence saved verbatim under evidence/ (r9_r2_*).
"""
...
def consumer_payload(out: str) -> str:
    ...
    minus the run-shape markers (mode, tsgrammar_importable,
    source_meta_agrees_with_byte_lines — the last is a FINDING, not a
    payload: it is False over flora in EVERY shape by design)."""
    d = json.loads(out)
...
def main() -> int:
    tmp = Path(tempfile.mkdtemp(prefix="phase9-run2-"))
    ...
    # 1. the fresh venv: ONLY the light wheels + tree-sitter-nix (real index)
    banner("1. fresh venv: light wheels + tree-sitter-nix from the real index")
    venv = tmp / "fresh-venv"
    ...
    print(f"  tree-sitter-nix wheel: {wheel_version!r}")
    ...
    shutil.copyfile(CONSUMER, env / "consumer_nix.py")
    ...
    def go(label: str, python: str, extra_env: dict, *args: str) -> str:
        e = {**os.environ, **extra_env} if label.startswith("inrepo") \
            else {**pyenv, **extra_env}
        p = run([python, str(env / "consumer_nix.py"), fleet, *args],
                cwd=tmp, env=e)
        if p.returncode != 0:
            print(f"  {label}: rc {p.returncode}\n{p.stdout}\n{p.stderr}")
            save(f"r9_r2_{label}.txt", f"rc {p.returncode}\n{p.stdout}\n{p.stderr}")
            raise SystemExit(f"{label} failed (rc {p.returncode})")
        (EVIDENCE / f"r9_r2_{label}.txt").write_text(p.stdout)
    ...
    print("Run 2:", "GO — the light install delivers the nix consumer in "

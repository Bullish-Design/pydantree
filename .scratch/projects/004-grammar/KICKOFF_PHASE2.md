# KICKOFF — pydantree Phase 2 (Product B core: `pydantree_sitter_grammar`)
...
> live in `.scratch/`; your findings go in `.scratch/004-pydantree_sitter_grammar/FINDINGS.md`.
...
This session runs **Phase 2 — Product B core (`pydantree_sitter_grammar`)**, per the concept
...
1. **IR** (`pydantree_sitter_grammar.ir`): the full node set from Phase-0 §3 as a Pydantic
...
- **External scanners.** The IR accepts `externals`; no scanner library, no
  scanner authoring.
- **wasm, packaging/wheels, package splitting** (`pydantree_sitter`/`pydantree_sitter_grammar`/
  `pydantree_sitter`), corpus-testing harness (Phase 5), incremental-reparse wrappers,
...
3. **Where the code goes:** this is the real build — create a proper package
   **`src/pydantree_sitter_grammar/`** (submodules: `grammar.py`, `builder.py`, `checks.py`,
   ...
   `tests/`. Add `src/pydantree_sitter_grammar` to `[tool.hatch.build.targets.wheel].packages`
   in `pyproject.toml` and reinstall the editable package so `import pydantree_sitter_grammar`
   works in the venv. Distribution/splitting is a later decision — organize so
   the future `pydantree_sitter`/`pydantree_sitter` split is mechanical, but build one package now.
...
- **Commit after each meaningful step**, with clear messages (e.g.
  `pydantree_sitter_grammar: IR mirrors full 0.25.3 schema, round-trips`,
  `pydantree_sitter_grammar: builder DSL + definition-site recording`,
  `pydantree_sitter_grammar: analyzer catches unused-rule pruning footgun`,
  `pydantree_sitter_grammar: pipeline generate→gcc→load→parse with caching`,
  `pydantree_sitter_grammar: conflict remapping ported, evidence captured`).
- **Write findings as you go** into `.scratch/004-pydantree_sitter_grammar/FINDINGS.md`. The
...
- **Don't fake the primary experiment.** Experiment A's reference and
  ...
  `.scratch/004-pydantree_sitter_grammar/evidence/`.
- **Ask before expanding scope** beyond this brief.

## Deliverables (end of session)

1. Working `src/pydantree_sitter_grammar/` package (IR + emitter, builder, analyzer, pipeline,
...
2. Demonstrated, with evidence:
   ...
   - (d) a deliberate conflict remapped to DSL `file:lineno` with suggested
     fixes.
3. `.scratch/004-pydantree_sitter_grammar/FINDINGS.md` answering at minimum:

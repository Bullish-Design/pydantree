...
Phase 4 — the bet-#2 bridge experiment (the go/no-go).
  ...
  RUN 1 — the pitch. A non-JSON config grammar (INI-like, pydantree_sitter_grammar-built)
...
shape-map metric:
  ...
  (pydantree_sitter/shapes.py shape_for: one generic kind-name inference, restricted by
...
def run3() -> None:
    ...
    from pydantree_sitter import typed as _typed
    ...
    from pydantree_sitter.typed import _value_specs  # the Phase-1 stand-in

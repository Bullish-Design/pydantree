...
Supporting: `tscore.schema` (the shared node-schema + two derivations),
`tsquery.shapes.shape_for` (Job 3), `tsquery.schema.check_model_schema` /
...
4. **Merged aliases** (multiple rules aliased under one name) union field
   ...
   overstated. Untouched by the experiment grammars; documented in tscore.
...
- **Phase 5 (corpus harness, distribution):** the corpus-testing harness
  ...
  the `semantic_smoke` helper is its seed. Distribution: `tscore`/`tsquery`

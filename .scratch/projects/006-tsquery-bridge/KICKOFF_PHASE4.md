...
> in `.scratch/006-tsquery-bridge/FINDINGS.md`.
...
Phase 3 (`.scratch/005-tsgrammar-glr/`) proved B's GLR-ergonomics layer and
...
- **Phase 3 (done, THIS phase's foundation):** `.scratch/005-tsgrammar-glr/` —
...
1. **`.scratch/002-pydantic-treesitter/CONCEPT.md`** — §5 (the validated
   ...
   the four jobs and the community-grammar derivation path)**, §8 (tscore is
...
4. **`.scratch/005-tsgrammar-glr/FINDINGS.md`** — Phase 3's verdict, and
...
5. **Phase-2 code you will extend (skim; base facts in the Phase-2 FINDINGS
   appendix):**
   - `src/tsgrammar/grammar.py` — the IR. The node-schema is derived from
   ...
   - `src/tsgrammar/pipeline.py` — `BuildResult.node_types_json` (the CLI's
     byproduct, already on disk and currently unused — Phase 4's
     community-grammar path samples it).
   - `src/tsgrammar/expressions.py` + `builder.py` (`Ladder`) — for the
     Phase-3A hardening items.
6. **The layer to port (read before writing any tsquery code):**
...
```
tsgrammar (B) ──► grammar.json ──► node-schema.json (derived from the IR)
                                        │                     │
   community grammars ──► node-types.json (CLI byproduct) ────┘ (weaker path)
                                        ▼
   tsquery (A): Language.load(schema=…) ──► class-creation checks (jobs 1,3,4)
...
small **non-JSON config grammar** with tsgrammar (INI-like: sections,
...
1. **tscore — the shared node-schema (keep it ONE small module, no more):**
   `src/tscore/schema.py` — Pydantic models mirroring `node-types.json`'s
   ...
   Wire `src/tscore` into `pyproject.toml`'s wheel packages list. tsgrammar
   and tsquery both import it; **A still never imports B**.
2. **B side:** `tsgrammar.pipeline.build()` emits `node-schema.json` alongside
...
3. **A side — port + integrate (`src/tsquery/`, the first real Product A
   package):**
   - port `spike-a2/typed.py` + the `spike-a` emitter/materializer substrate
     mechanically into `src/tsquery/` (same surface: `OutputModel`,
...
- **wasm distribution, wheel/package splitting beyond the one `src/tscore`
...
2. **Verified facts (don't re-derive):** tree-sitter CLI 0.25.3, bindings
   ...
   `src/tsgrammar/pipeline.py` + Phase-2 FINDINGS appendix.
3. **Wheels:** `tree-sitter-python` and `tree-sitter-json` are installed in the
   ...
   them). `import tsgrammar` works (editable install); the spike-a2 layer runs
   via a sys.path shim — the port into `src/tsquery/` removes that shim.
4. **The CLI's `node-types.json` byproduct** is already produced by every
   `tsgrammar` build (see `BuildResult.node_types_json`); Phase 3 left it on
...
- **Commit after each meaningful step** (e.g.
  `tscore: node-schema models + derive_from_ir (exact) and derive_from_node_types (community) converge`,
  `tsgrammar: build() emits node-schema.json alongside grammar.json`,
  `tsquery: port spike-a2 model-only layer (surface frozen, tests green)`,
  `tsquery: schema jobs 1+4 — model↔grammar and capture↔type checks at class creation`,
  `tsquery: job 3 — derived record value-shape map replaces the hardcoded JSON map`,
  `tsquery: record-level anchoring kills the nested-collision class`,
  `tsgrammar: Phase-3A — ExpressionGrammar semantic-smoke corpus + cond=/non_call_primary`,
  `phase4: experiment — bet #2 bridge verdict, evidence captured`).
- **Write findings as you go** into `.scratch/006-tsquery-bridge/FINDINGS.md`.
...
- **Don't fake the primary experiment.** Run 1's corpus, Run 2's planted
  ...
  `.scratch/006-tsquery-bridge/evidence/`.
- **Ask before expanding scope** beyond this brief.

## Deliverables (end of session)

1. Working Phase-4 extensions, all committed:
   - `src/tscore/schema.py` (node-schema models + both derivation paths,
     wired into the wheel packages);
   - `src/tsgrammar/` emits `node-schema.json` in `BuildResult`;
   - `src/tsquery/` — the ported model-only layer with Jobs 1, 3, 4, and
...
3. `.scratch/006-tsquery-bridge/FINDINGS.md` answering at minimum:
...
9. **Package layout today:** `pyproject.toml` wheel packages =
   `["src/pydantree", "src/examples", "data", "src/tsgrammar"]`; Phase 4 adds
   `src/tscore` and `src/tsquery`. `tsgrammar` is editable-installed;
   `import tsgrammar` works in the devenv venv.

...
> Phase 4 (`.scratch/006-tsquery-bridge/`) proved the **bridge** and delivered
...
> `.scratch/007-tsquery-distribution/FINDINGS.md`.
...
2. **`.scratch/002-pydantic-treesitter/CONCEPT.md`** — §4.8 (the corpus-testing
   ...
   path), §8 (tscore owns the artifact-loading contract — a Phase-5 fact).
...
5. **`.scratch/005-tsgrammar-glr/FINDINGS.md`** — §6 (the corpus harness is the
...
6. **Phase-4 code you will extend (skim):**
   - `src/tsgrammar/expressions.py` — `semantic_smoke` (the seed; the harness
     should subsume or call it), `DEFAULT_PRECEDENCE_CORPUS`.
   - `src/tsgrammar/pipeline.py` — `BuildResult` (add packaging), the cache,
     `build_loop`.
   - `src/tsquery/typed.py` — `Language` (add reparse), `_resolve_language`,
     `_ExtractionError` (richen it), `_record_kwargs` (the anchor-merge
     machinery field-mode lists will reuse).
   - `src/tsquery/dsl.py` — `Query.validate` (currently returns dicts; make
     typed `Diagnostic`s), the emitter (for `...`/descendant + list captures).
   - `src/tscore/schema.py` — the derivations (the community tool consumes
     `derive_from_node_types`).

## Phase 5 in 60 seconds

```
   corpus testing (B)                      distribution (B -> tscore -> A)
...
**Run 2 — the artifact seam in production (B → tscore → A).** Build a grammar
...
`sys.path`** (and does not import `tsgrammar` at all): `tsquery` loads the
...
1. **Corpus testing (B, the centerpiece):** `tsgrammar/corpus.py` —
...
2. **Artifact packaging (B + tscore + A):** `BuildResult.package(dir)` (or a
   small `tsgrammar.package(result, dir)`) emits the bundle:
   `grammar.so`, `node-schema.json`, `tree-sitter.json` (metadata), and a
   `loader.py` (a few lines: ctypes → PyCapsule → `tree_sitter.Language` —
   the loader **belongs in `tscore`** as the shared artifact-loading contract,
   CONCEPT §8, so both B's `tsgrammar.language` and A can use it without
   importing each other). A side: `tsquery` consumes the bundle — `Language.load`
   already accepts a language + schema; make loading a bundle directory a
   one-liner (a `tsquery.language`-level helper or a `Language.load_bundle(dir)`).
   **A must not gain a tsgrammar dependency** — the loader and schema are the
   only imports.
3. **Community-schema tool (small):** given a grammar source dir containing a
   `grammar.json` (or a tsgrammar IR), run the CLI generate (which produces
...
2. **Verified facts (don't re-derive):** tree-sitter CLI 0.25.3, bindings
   ...
   All in `src/tsgrammar/pipeline.py` + Phase-2/4 FINDINGS appendices.
3. **Wheels:** `tree-sitter-python` and `tree-sitter-json` are installed in
   ...
   `import tsgrammar`, `import tsquery`, `import tscore` all work (editable
   install; `src/` is on the path via the devenv).
4. **Phase-4 fixtures you will reuse:** `.scratch/006-tsquery-bridge/` has
...
- **Commit after each meaningful step** (e.g.
  `tsgrammar: corpus harness — (source, expected-sexp) cases, CST render + diff, grammar/node-schema snapshots; semantic_smoke delegates to it`,
  `tscore: artifact loader (PyCapsule) as the shared loading contract; BuildResult.package() emits grammar.so + node-schema.json + metadata + loader`,
  `tsquery: consume a packaged bundle in a B-free subprocess — checks active, ground truth passes`,
  `tsgrammar: community-schema tool (node-types.json -> node-schema.json)`,
  `tsquery: reparse + typed Diagnostics (kind/span/expected/snippet)`,
  `tsquery: ExtractionError per-match detail + descendant M(...) via has-ancestor + field-mode lists via anchor-merge + Unescaped()`,
  `tsgrammar: scanner escape hatch airtight + indentation scanner seed`,
  `phase5: experiment — reach verdict (corpus bite + B-free bundle + control), evidence captured`).
- **Write findings as you go** into `.scratch/007-tsquery-distribution/FINDINGS.md`.
...
- **Don't fake the primary experiment.** Run 1's planted regressions, Run 2's
  ...
  raw outputs verbatim under `.scratch/007-tsquery-distribution/evidence/`.
- **Ask before expanding scope** beyond this brief.

## Deliverables (end of session)

1. Working Phase-5 extensions, all committed:
   - `src/tsgrammar/corpus.py` (harness + snapshots; `semantic_smoke` delegates);
   - artifact packaging (`BuildResult.package()` or `tsgrammar.package`) +
     the loader in `src/tscore/` (shared loading contract) + A's
     bundle-consumption helper;
   - the community-schema tool;
   - `src/tsquery/` — `reparse` + typed `Diagnostic`s + the polish items that
...
3. `.scratch/007-tsquery-distribution/FINDINGS.md` answering at minimum:
...
1. **The bridge is proven (Phase 4).** `validate_with(language, schema=)`
   ...
   model site cited; the record shape map is derived (`tsquery.shapes.shape_for`
   ...
   unchanged. Evidence: `.scratch/006-tsquery-bridge/evidence/`.
2. **The node-schema derivation is exact.** `derive_from_ir` matches the CLI's
   ...
   is equivalent on the shared subset. `tscore.schema.NodeSchema` has the query
...
3. **The artifact seam is one artifact.** A never imports B; the only coupling
   ...
   loader to `tscore` (CONCEPT §8: tscore owns the artifact-loading contract).
...
5. **`semantic_smoke` is the corpus seed.** `src/tsgrammar/expressions.py`:
...
8. **Phase-4 fixtures:** `cfg_grammar.py` (15 rules, analyzer CLEAN, ABI 15,
   ...
   `["src/pydantree", "src/examples", "data", "src/tsgrammar", "src/tscore",
   "src/tsquery"]`.

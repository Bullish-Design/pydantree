...
Consumes a packaged grammar bundle in a SEPARATE process where tsgrammar is
NOT importable (its only path — the editable src/ install — is stripped by
consumer_env/sitecustomize.py). A never imports B; the bundle + tscore's
...
try:
    ...
    print(json.dumps({"ok": False,
                      "error": "tsgrammar IS importable — B leaked into the "
                               "consumer process"}))
    sys.exit(1)

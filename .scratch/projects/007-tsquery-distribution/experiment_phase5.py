...
Phase 5 — the reach experiment (polish & reach: corpus + distribution).
  ...
  RUN 2 — the artifact seam in production (B -> tscore -> A). The cfg grammar
          is packaged (BuildResult.package) into a bundle; a SEPARATE process
          where tsgrammar is NOT importable consumes it via
...
def run1() -> None:
    ...
    from tsgrammar.grammar import ChoiceNode, SymbolNode
    gb = qfilter.build()
...
def run2() -> None:
    banner("RUN 2 — the artifact seam in production (B -> tscore -> A, "
           "B-free subprocess)")
    ...
    data = json.loads(out)
    ok = rc == 0 and data["ok"]
    print(f"2.2 B-free subprocess consumer (tsgrammar NOT importable): "
...
def run3() -> None:
    ...
    # .scm + manual dispatch + manual coercion — no tsquery, no schema
    import ctypes
    import tree_sitter

...
> Phase 5 (`.scratch/007-tsquery-distribution/`) proved the reach — corpus
...
> built — today ONE distribution ships tsgrammar+tscore+tsquery together), prove
...
it **exposed a real leak** (tscore imported tsgrammar at module level; fixed by
`tscore._ir_derive`). This session delivers **Phase 6 — the consumer seam**:
...
1. **The packaging seam (CONCEPT §8, never built):** can a user install
   **tscore + tsquery only** (light, no toolchain) and get the full checked
   extraction — while **tsgrammar** (heavy: Rust CLI + C toolchain) ships
   ...
   tsgrammar) installs the light distributions and the Phase-5 bundle
...
If the light install leaks tsgrammar, or the community tool disagrees over a
...
- **Phases 0–4 (done):** emission + conflict remapping; Product A's model-only
  ...
  touch**.
- **Phase 5 (done, THIS phase's foundation):** `.scratch/007-tsquery-distribution/` —
  the corpus harness (`tsgrammar/corpus.py`, `semantic_smoke` delegates), the
  artifact seam (`tscore/loader.py` = the shared loading contract,
  `BuildResult.package()` → 4-file bundle, `tsquery.Language.load_bundle`), the
  community-schema tool (`tsgrammar/schema_tool.py`), A polish (reparse +
  typed `Diagnostic`, per-match `ExtractionError`, descendant `...`,
  field-mode lists, `Unescaped()`), the airtight scanner escape hatch +
  indentation-scanner seed (`tsgrammar/scanners/`), and the B-free subprocess
...
- **Packaging (the gap this phase closes):** `pyproject.toml` publishes ONE
  ...
  "data", "src/tsgrammar", "src/tscore", "src/tsquery"]` and
  `tree-sitter>=0.23`. **No wheel has ever been built** (the devenv venv has no
  pip — `uv` is the manager). The CONCEPT §8 layout (tscore tiny shared,
  tsquery light, tsgrammar heavy) is code-only; the distribution has never
  matched it.

## Required reading (in this order — do not skip)

1. **`.scratch/007-tsquery-distribution/FINDINGS.md`** — Phase 5's verdict, §4
...
2. **`.scratch/002-pydantic-treesitter/CONCEPT.md`** — §8 (the distribution
   strategy this phase builds: tscore tiny pure-Python + the artifact-loading
   contract, tsquery light runtime with no Rust CLI/compiler, tsgrammar heavy
   ...
   distribute pipeline).
3. **`.scratch/006-tsquery-bridge/FINDINGS.md`** — §4 (Job 2 assessed-not-built:
...
5. **Phase-6 code you will extend (skim, then read the parts you touch):**
   ...
   - `src/tsquery/typed.py` — `_SCHEMA_REGISTRY: dict[str, object]` keyed by
   ...
   - `src/tscore/schema.py` + `src/tscore/loader.py` + `src/tscore/_ir_derive.py` —
     the B-free seam (verified); the schema's per-kind children/fields are the
     Job-2 stub source.
   - `src/tsgrammar/schema_tool.py` — the community tool (Run 2's subject).
   - `src/tsgrammar/scanners/` — `indent_scanner.c` (the seed), `scanner_for()`
     (the library table to grow).
   - `.scratch/007-tsquery-distribution/bfree.py` + `consumer*.py` + the
...
```
   ...
   one dist -> tscore + tsquery (light)     real grammar SOURCE (rust/python)
   tsgrammar (heavy) separate               -> schema tool -> node-types.json
...
`tscore` + `tsquery` install WITHOUT `tsgrammar`, and `tsgrammar` (the heavy
...
carries tsgrammar + the scanner package data (`scanners/indent_scanner.c` must
ride in the wheel), and (c) **the dev flow keeps working** — the tests import
`tsgrammar`/`tsquery`/`tscore` from `src/` via the editable install, so either
...
deps, assert `import tsgrammar` fails, run the Phase-5 bundle round-trip
...
install's import graph (`tsquery` → `tscore` → pydantic/tree_sitter, never
tsgrammar), and the byte-identical A surface vs the in-repo results. **The
go/no-go: a consumer who installs only the light packages gets the full
checked extraction, and B's toolchain stays out — or the seam leaks (tsgrammar
...
source). Run `tsgrammar.schema_tool` over it; the tool's contract is
...
2. **Verified facts (don't re-derive):** tree-sitter CLI 0.25.3, bindings
   ...
   `tscore.loader.load_grammar_so`. **NEW for this phase:** the devenv venv has
...
3. **Wheels:** `tree-sitter-python` and `tree-sitter-json` installed in the
   devenv venv. `import tsgrammar`, `import tsquery`, `import tscore` all work
   (editable install; a `.pth` in site-packages points at `src/` — that entry
   is what the B-free sitecustomize strips).
4. **Phase-5 fixtures you will reuse:** `.scratch/007-tsquery-distribution/` —
   ...
   the cfg/json grammars in `.scratch/006-tsquery-bridge/`, the bundle
...
5. **Before writing helpers:** re-run `python -m pytest tests/` (139 green at
   phase end) and `.scratch/007-tsquery-distribution/experiment_phase5.py`,
...
- **Commit after each meaningful step**, e.g.:
  `packaging: split distributions — tscore/tsquery light installables, tsgrammar heavy; tree-sitter>=0.26 pin; scanner .c rides as package data; dev flow (editable) still covers the suite`,
  `phase6: fresh-venv harness (uv) — light install, tsgrammar unimportable, bundle round-trip + community extraction byte-identical`,
  `tsgrammar: community seam over tree-sitter-rust (or python) — schema tool agreement + B-free extraction vs hand truth`,
  `tsquery: Job-2 .pyi stubs from the schema (typed per-kind accessors)`,
  `tsgrammar: scanner library seeds (heredoc/matched-delimiter) + tests`,
  `tsquery: fix the schema-registry name-keyed global leak (per-Language scoping, convenience preserved)`,
...
1. Working Phase-6 extensions, all committed:
   - the distribution split (light `tscore`+`tsquery` installables; `tsgrammar`
   ...
   - the fresh-venv harness + end-to-end test (light install → tsgrammar
...
1. **The artifact seam is one artifact + one loading contract.** A never
   ...
   + a 7-line `loader.py` delegating to `tscore.loader.load_bundle`.
   `Language.load_bundle(dir)` is the one-line consumer. Verified B-free in a
   subprocess with the editable `src/` install stripped.
2. **tscore is B-free at import time** (the Phase-5 fix): the exact-path
   derivation lives in `tscore._ir_derive`, imported only when
   `derive_from_ir` is called (B-side only); `import tscore` / `import
   tsquery` never touch tsgrammar — the packaging split can rely on this.
...
5. **The editable-install mechanics**: a `.pth` in site-packages points at
   ...
   containing `pydantree/src`, with `tscore`/`tsquery` copied into the
   consumer env's `lib/`. The consumer asserts `import tsgrammar` fails.
...
7. **The registry leak is real and documented.** `_SCHEMA_REGISTRY` in
   `tsquery/typed.py` is keyed by language name: a schema bound via
...
9. **Package layout:** the six packages are `src/pydantree` (legacy, frozen),
   `src/examples`, `data`, `src/tsgrammar`, `src/tscore`, `src/tsquery`;

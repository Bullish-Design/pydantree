...
`src/tsquery/dsl.py:244` (`Query.compile` caches `self._compiled` ignoring
`lang`) + `src/tsquery/typed.py:640` (`_derived_cache` is class-level).
...
`src/tsquery/typed.py:996-1006` (`_record_kwargs`): nested models recurse
...
`src/tsquery/typed.py:426` (`k = kind.kinds[0]`) and
`src/tsquery/schema.py:469` (`return override[0]`). `NodeKind(("true",
...
`src/tsquery/stubs.py` emits `.pyi` accessors (`fn.name()`, `.get("name")`,
...
`src/tsgrammar/builder.py:363-364`: appends the alias NAME to the
...
`src/tsgrammar/rules.py:312` top-level: raises raw `ValueError: too many
...
(`src/tsgrammar/rules.py:83`).

## B4 — Author's own whitespace extra doesn't suppress the `\s` default
`src/tsgrammar/builder.py:428-430` only recognizes the exact pattern `\s`;
...
- `schema_derive` cache keyed by `language_name or "?"`
  ...
  never re-derived.
- `tsquery.Language.load_bundle` (`typed.py:729-735`) drops `bundle.lib`,
  violating tscore.loader's own stated keep-alive contract
...
- Two distinct `ExtractionError` classes exist (`typed.py:1055`,
  ...
  `tsquery.ExtractionError` is the typed one — `except tsquery.ExtractionError`
...
- `tscore/__init__.py` docstring: "One module, no more: tscore.schema" —

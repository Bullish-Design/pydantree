...
1. **`scratch/018-pydantic-treesitter/CONCEPT.md`** — THE authoritative concept.
...
2. **`scratch/017-pydantic-winnow-parser/SESSION_ANALYSIS.md`** — the previous
...
3. **`scratch/017-pydantic-winnow-parser/sketch.py`** — a self-contained
...
4. (Optional) `scratch/017-pydantic-winnow-parser/WINNOW_PYO3_PYTHON_COMBINATORS_CONCEPT.md`
...
- **Spike code goes in `spike/`** at the repo root, isolated from `src/`. Reference
  docs stay in `scratch/`.
...
- **Write findings as you go** into `spike/FINDINGS.md`. The code is throwaway;
...
1. Working code in `spike/`: IR → `grammar.json` → generated + compiled parser
   that parses the sample expression language with correct precedence.
2. `spike/FINDINGS.md` answering at minimum:

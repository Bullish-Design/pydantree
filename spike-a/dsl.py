"""Query DSL -> .scm emitter for tree-sitter 0.26 (Product A spike).

Design notes (all verified against the installed 0.26 bindings):
  ...
  * A `NodeSpec` tree emits one S-expression pattern. `.where()` predicates
    ...
    is parsed as a SECOND, empty pattern that matches every node — probe
    finding).
  ...
  * There is NO inline alternation in tree-sitter queries: `a | b` emits two
    top-level patterns (each its own pattern index). `Query.matches()` reports
    the pattern index, and `capture_quantifier(pi, ci)` is per-pattern.
  * Quantifiers `*`/`+`/`?` on a sub-node: the DSL never captures the
    quantified node itself (untested/ambiguous syntax); captures live on the
    children (e.g. `(array (string (string_content) @tag)*)`), which the
    bindings mark with the matching quantifier.

Run: devenv shell -- python spike-a/dsl.py   (self-test)
"""
...
if __name__ == "__main__":
    ...
    q = Query(
        ...
        .child(node("expression_statement")
               .child(node("assignment")
                      .child(field="left", node=node("identifier").capture("name"))
                      .child(field="right", node=node("integer").capture("value")))
               .capture("stmt"))
        .capture("root")
    ...
    compiled = q.compile(lang)
    print("compiled OK: patterns =", compiled.pattern_count,
          "captures =", compiled.capture_count)

    tree = tree_sitter.Parser(lang).parse(
        b"WIDTH = 1920\nlow = 1\nHEIGHT = 1080\n")
    cur = q.run(tree)
    for m in cur.matches():
        print(" ", m.text("name"), "=", m.text("value"),
              "line", m.first("stmt").line, "quant(name):",
              m.quantifier("name"))

    # alternation -> two patterns
    a = node("integer").capture("v") | node("float").capture("v")
    q2 = Query(a)
    print("\nalternation patterns:")
    print(q2.source)
    print("compile:", q2.compile(lang).pattern_count, "patterns")

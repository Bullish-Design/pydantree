"""Capture -> OutputModel materialization (Product A, concept §5.4).
...
Design (verified against pydantic 2.13 / tree-sitter 0.26):
  ...
  * Pydantic v2 lax mode is the coercion engine: we hand the materializer
    ...
    "1920" -> int, "98.5" -> float, "true" -> bool, "admin" -> enum, and
    raise a clean `ValidationError` (with per-field loc) for malformed input.
  ...
  * Repeated captures -> list: a `list[X]` field collects every node captured
    ...
    captures in one match — the cursor yields one match per occurrence, so the
    record extractor merges captures across matches sharing a record anchor.)
  * Multiple nodes feeding one scalar field = AmbiguousCaptureError (a nested
    structure with the same key as a wanted field).
"""

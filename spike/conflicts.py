"""
Conflict -> Python-source remapping (the Phase 0 go/no-go experiment).
...
`variable_name` maps 1:1 to a grammar rule we recorded at build time, so we can
raise a GrammarConflictError naming the author's `g.rule(...)` call sites.
"""
...
def remap_from_proc(grammar: Grammar, proc, *, raw_path: str | None = None):

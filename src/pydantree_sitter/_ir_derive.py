"""pydantree_sitter._ir_derive — the exact-path node-schema derivation (derive_from_ir).

Kept OUT of pydantree_sitter.schema's import graph: it imports pydantree_sitter_grammar.ir (B's
IR), so it is only importable when B is present. pydantree_sitter.schema exposes
`derive_from_ir` as a lazy function that imports this module on first call —
A (pydantree_sitter consumers, the B-free bundle path) imports pydantree_sitter.schema without
ever touching pydantree_sitter_grammar; the exact-path derivation is B-side machinery.

The derivation mirrors node_types.rs (see pydantree_sitter.schema's module docstring).
"""
...
from pydantree_sitter_grammar.ir import (  # noqa: E402  (this module is B-side only)
    AliasNode,
    BlankNode,
    ChoiceNode,
    FieldNode,

...
pydantree_sitter + the tree-sitter CLI + a C toolchain; produces the artifacts A
consumes (CONCEPT §8).
...
Also ships: the RULE-CLASS surface (`pydantree_sitter_grammar.rules` — each rule is a
...
airtight mechanism), the corpus harness (`pydantree_sitter_grammar.corpus`), and the
community-grammar schema tool (`pydantree_sitter_grammar.schema_tool`).

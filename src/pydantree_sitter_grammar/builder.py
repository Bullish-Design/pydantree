"""
pydantree_sitter_grammar.builder — the authoring DSL that emits Grammar IR nodes.

Every combinator is sugar; every operator lands on the same validated
`pydantree_sitter_grammar.ir.Rule` nodes. `Grammar` is a registry of named rules +
...
(`pydantree_sitter_grammar.conflicts`) able to point at the author's Python source.

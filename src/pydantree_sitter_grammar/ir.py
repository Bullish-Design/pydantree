"""
pydantree_sitter_grammar.ir — the canonical IR mirroring tree-sitter's real
`grammar.json` schema.
...
`type`; a `GrammarModel` is the top-level container. Because both sides are
Pydantic, `GrammarModel.model_dump_json()` *is* `grammar.json` and
`GrammarModel.model_validate_json()` round-trips it — which is what makes importing

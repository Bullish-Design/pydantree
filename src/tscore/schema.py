...
def derive_from_ir(grammar) -> list[NodeTypeInfo]:
    """The exact path: walk a tsgrammar Grammar IR -> the canonical
    node-schema list (mirrors the CLI's node-types.json).

    Phase 5: the implementation lives in `tscore._ir_derive` and imports
    tsgrammar.grammar (B's IR) — so calling this REQUIRES B to be installed.
    It is imported lazily here so `import tscore` / `import tsquery` stay

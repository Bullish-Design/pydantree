"""tsgrammar.corpus — the corpus-testing harness (CONCEPT §4.8, Phase 5).
...
Rendering has a documented normalization story (the anonymous-node decision):
  ...
  * ``style="compact"`` — the Phase-3A semantic-smoke format (``expr`` nodes
    ...
    ``tsgrammar.expressions.semantic_smoke`` delegates here — no parallel
    machinery.
...
class Corpus:
    ...
    def run(self, build_result=None, grammar: Grammar | None = None, *,
        ...
        Builds `grammar` (a tsgrammar builder Grammar) via the pipeline, or

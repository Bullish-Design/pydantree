...
where pydantree_sitter_grammar is genuinely NOT importable:

  * a fresh consumer env dir gets a copy of `pydantree_sitter` (A's own
    packages — never pydantree_sitter_grammar);
  * a `sitecustomize.py` (this directory's consumer_env/) strips the editable
    ...
    path to pydantree_sitter_grammar is gone;
  * the consumer script itself asserts `import pydantree_sitter_grammar` raises
...
ROOT = Path(__file__).resolve().parents[3]  # tests/fixtures/bfree -> repo root
SRC = ROOT / "src"
